...
function Home({ user }) {
  ...
  async function fetchAndPlay(song) {
    ...
    try {
      ...
      console.log("YouTube fetch error:", e);
    }
  }
  ...
  function ProgressBar() {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        ...
        <div
          style={{ flex: 1, height: "4px", background: "#404040", borderRadius: "2px", cursor: "pointer" }}
          onClick={(e) => {
  ...
  function SongRow({ song, index, queue, playlistId }) {
    return (
      <div
        style={{ display: "flex", alignItems: "center", gap: "12px", padding: "8px 12px", borderRadius: "6px", background: currentSong?.trackId === song.trackId ? "#ffffff15" : "transparent", transition: "background 0.15s", cursor: "pointer" }}
      ...
      >
        {index !== undefined && (
          <div style={{ width: "20px", textAlign: "center", color: "#b3b3b3", fontSize: "13px", flexShrink: 0 }}>{index + 1}</div>
        )}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div onClick={() => openArtist(song)} style={{ fontSize: "13px", color: "#b3b3b3", marginTop: "2px", cursor: "pointer" }}>{song.artistName}</div>
        </div>
        <button onClick={(e) => { e.stopPropagation(); toggleLike(song); }} style={{ background: "none", border: "none", fontSize: "16px", cursor: "pointer", flexShrink: 0, opacity: isLiked(song) ? 1 : 0.4 }}>
          {isLiked(song) ? "❤️" : "🤍"}
        </button>
        <button onClick={(e) => { e.stopPropagation(); setShowAddToPlaylist(song); }} style={{ background: "none", border: "none", fontSize: "16px", cursor: "pointer", flexShrink: 0, opacity: 0.4, color: "white" }}>➕</button>
        {playlistId && (
          <button onClick={(e) => { e.stopPropagation(); removeFromPlaylist(playlistId, song.trackId); }} style={{ background: "none", border: "none", fontSize: "16px", cursor: "pointer", flexShrink: 0, opacity: 0.4, color: "white" }}>🗑️</button>
        )}
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "#000", color: "white", fontFamily: "Inter, sans-serif" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "#121212", zIndex: 200, display: "flex", flexDirection: "column", overflowY: "auto" }}>
          ...
          <div style={{ padding: "16px 32px", display: "flex", justifyContent: "center", flexShrink: 0 }}>
            <img
              ...
              style={{ width: "100%", maxWidth: "280px", borderRadius: "8px", boxShadow: "0 32px 64px rgba(0,0,0,0.8)" }}
            />
          </div>
          ...
          <div style={{ padding: "20px 24px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
            ...
            <button onClick={() => toggleLike(currentSong)} style={{ background: "none", border: "none", fontSize: "26px", cursor: "pointer", flexShrink: 0, marginLeft: "16px" }}>
          ...
          <div style={{ padding: "0 24px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
            ...
            <button
              ...
              style={{ width: "60px", height: "60px", borderRadius: "50%", background: "#fff", border: "none", color: "black", fontSize: "22px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {isPlaying ? "⏸" : "▶"}
            </button>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              {relatedSongs.map((song) => (
                <div
                  ...
                  style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px", borderRadius: "6px", cursor: "pointer", background: currentSong?.trackId === song.trackId ? "#ffffff15" : "transparent" }}
                >
                  ...
                  <span style={{ fontSize: "14px", color: "#b3b3b3", flexShrink: 0 }}>▶</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {showAddToPlaylist && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div style={{ background: "#282828", borderRadius: "8px", padding: "24px", width: "100%", maxWidth: "360px" }}>
            ...
            {playlists.length === 0 ? (
              <p style={{ color: "#b3b3b3", fontSize: "14px", marginBottom: "16px" }}>No playlists yet. Create one first!</p>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "16px" }}>
                {playlists.map((p) => (
                  <div key={p.id} onClick={() => addToPlaylist(p.id, showAddToPlaylist)} style={{ padding: "12px 16px", background: "#3e3e3e", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>
      ...
      <div style={{ background: "#000", padding: "16px 20px", display: "flex", alignItems: "center", gap: "12px", position: "sticky", top: 0, zIndex: 10, borderBottom: "1px solid #282828" }}>
        {(page === "artist" || page === "playlist" || page === "liked") && (
          <button onClick={() => setPage(page === "artist" ? "search" : "library")} style={{ background: "none", border: "none", color: "white", fontSize: "22px", cursor: "pointer", flexShrink: 0 }}>←</button>
        )}
          ...
          <>
            <div style={{ flex: 1, position: "relative" }}>
              <input
                ...
                onKeyDown={handleKey}
                style={{ width: "100%", padding: "10px 16px 10px 40px", borderRadius: "50px", border: "none", background: "#242424", color: "white", fontSize: "14px", outline: "none" }}
              />
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "12px", overflowX: "auto", paddingBottom: "8px" }}>
              {["Adele", "Taylor Swift", "Drake", "BTS", "Ed Sheeran", "Billie Eilish", "The Weeknd", "Ariana Grande"].map((a) => (
                <div key={a} onClick={() => { setQuery(a); setTimeout(() => searchSongs(), 100); }} style={{ flexShrink: 0, padding: "8px 16px", borderRadius: "50px", background: "#242424", cursor: "pointer", fontSize: "13px", fontWeight: "500", color: "white", whiteSpace: "nowrap" }}>{a}</div>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{ position: "fixed", bottom: "60px", left: 0, right: 0, background: "#282828", borderTop: "1px solid #3e3e3e", padding: "10px 16px", display: "flex", flexDirection: "column", gap: "8px", zIndex: 50 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
              <button onClick={() => toggleLike(currentSong)} style={{ background: "none", border: "none", fontSize: "18px", cursor: "pointer" }}>
                {isLiked(currentSong) ? "❤️" : "🤍"}
              </button>
              ...
              <button
                ...
                style={{ width: "38px", height: "38px", borderRadius: "50%", background: "#fff", border: "none", color: "black", fontSize: "15px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {isPlaying ? "⏸" : "▶"}
              </button>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, height: "60px", background: "#000", borderTop: "1px solid #282828", display: "flex", alignItems: "center", justifyContent: "space-around", zIndex: 50 }}>
        <div onClick={() => setPage("home")} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", cursor: "pointer", color: page === "home" ? "#fff" : "#b3b3b3" }}>
          <span style={{ fontSize: "20px" }}>🏠</span>
          <span style={{ fontSize: "10px", fontWeight: page === "home" ? "700" : "400" }}>Home</span>
        </div>
        <div onClick={() => setPage("search")} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", cursor: "pointer", color: page === "search" ? "#fff" : "#b3b3b3" }}>
          <span style={{ fontSize: "20px" }}>🔍</span>
          <span style={{ fontSize: "10px", fontWeight: page === "search" ? "700" : "400" }}>Search</span>
        </div>
        <div onClick={() => setPage("library")} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", cursor: "pointer", color: page === "library" ? "#fff" : "#b3b3b3" }}>
          <span style={{ fontSize: "20px" }}>📚</span>
          <span style={{ fontSize: "10px", fontWeight: page === "library" ? "700" : "400" }}>Library</span>
        </div>
        <div onClick={() => setPage("profile")} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", cursor: "pointer", color: page === "profile" ? "#fff" : "#b3b3b3" }}>
          <span style={{ fontSize: "20px" }}>👤</span>
          <span style={{ fontSize: "10px", fontWeight: page === "profile" ? "700" : "400" }}>Profile</span>
        </div>
      </div>
...
function App() {
  ...
  if (checking) return (
    <div style={{ background: "#000", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <h2 style={{ color: "#1db954", fontFamily: "Inter, sans-serif", fontWeight: "700" }}>🎵 MyMusic</h2>
    </div>
  );
